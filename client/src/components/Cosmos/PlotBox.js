...
*/
  constructor(props){
    ...
    plotTooltip(props){
      var time = new Date(props.label).toLocaleString('en-US')
      ...
      if(data.length>0) {
        ...
        <h4 style={{textAlign:'center'}}> {this.props.info.plot_title} </h4>
              <ResponsiveContainer width="100%" height={400}>
                ...
                <LineChart data={data}>
                  <XAxis dataKey="date"  type = 'number' allowDataOverflow={true} domain={this.props.plot_domain}
                  ...
                  <Tooltip labelFormatter={(val) => { return convertTimetoDate(val)}}/>
                  {lines}
                </LineChart>
                </ResponsiveContainer>
                </div>
