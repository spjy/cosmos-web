...
import {
  Card, Button, Form, Input, Modal, Icon, Select, Alert
} from 'antd';
...
class CosmosTools extends Component {
    ...
    if (agent_name !== '') {
      if (agent_list[agent_name]) {
        ...
        setup_agent(cosmosAgent).then((result) => {
          ...
          this.setState({ widgets, agents: agent_list });
          // console.log(e.form)
          if (e.form.widget_type === widgetType.LIVE_PLOT || e.form.widget_type === widgetType.COSMOS_DATA) {
            this.startListening(agent_name);
          }
  ...
  newAgent(cosmosAgent) {
    ...
    let agent_list = this.state.agents;
    let agent_name = cosmosAgent.agent;
    agent_list[agent_name] = { info: cosmosAgent, data: {} };
  ...
  render() {
    ...
    if (this.state.widgets.length > 0) {
      ...
      saveButton = (
        <Button type="default" onClick={this.showSaveModal.bind(this)}>
          <Icon type="save" />
          Save Widgets
        </Button>
      );
    ...
    let modal_buttons = [ <Button key="back" onClick={this.cancelSave.bind(this)}>Cancel</Button>,
      ...
      <div>
        ...
        <div>
          ...
          <Card size="small" style={{ width: '100%' }}>
            ...
            {widget}
            <Button type="default" onClick={this.onClickAddWidget.bind(this)}>
              <Icon type="plus" />
              {' '}
              New Widget
              {' '}
