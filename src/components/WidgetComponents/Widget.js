...
import {
  Button, Icon, Modal, Layout, Table, Alert, Col, Row
} from 'antd';
...
class Widget extends Component {
  ...
  onSaveForm() {
    ...
    if (this.validateForm()) {
      this.props.updateWidget({ form, id: this.props.id });
  ...
  render() {
    ...
    var table_data;
    // const table_cols = []
    let widgetTitle = `[${this.props.info.node}]${ this.props.info.agent}`;
      ...
      <Layout style={widget_style}>
        ...
        <Content>
          <div style={{ margin: '10px' }}>
            {' '}
            <p style={{ display: 'inline' }}><b>{widgetTitle}</b></p>
